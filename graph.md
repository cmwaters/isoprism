...
Go receiver ownership is also persisted as `edge_kind = 'owns_method'`. The receiver type is the source and the method is the destination, so `rpc/grpc:coregrpc.BlockAPI` links to `rpc/grpc:coregrpc.BlockAPI.Stop` even when the struct declaration itself is unchanged. Struct field type relationships are persisted as `edge_kind = 'uses_type'`; for example, a field like `BlockAPI.env *core.Environment` creates a graph edge from `rpc/grpc:coregrpc.BlockAPI` to `rpc/core:core.Environment` when that field type resolves to an indexed code node. Type-node Contents panels use `owns_method` edges for their method list and `uses_type` edges for the type dependencies that can be traversed.
...
or from the endpoints of a clicked edge. Clicking a node always selects it. On
the first click for a visible node, the client also requests hidden direct
callers and callees. Clicking a PR graph edge runs the same expansion request
for both edge endpoints and merges the results before the layout relaxes. This
does not rely solely on the `boundary` flag because PR graphs can filter
initial context edges before the expansion endpoint reloads the wider graph.
After a node has been expanded once, the client requests it again only when the
API reports `has_more`:
