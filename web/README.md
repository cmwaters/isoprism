...
Graph responses are function-level plus type nodes. The API returns canonical `full_name` values, but the UI splits them into a pink package/receiver label and a black title that contains only the function or method name. Function and method nodes expose `inputs[]`/`outputs[]` as structured `{name?, type, node_id?}` records. Struct field type relationships are represented as `uses_type` graph edges from the struct node to the indexed type node so they participate in expansion and boundary behavior. Go struct fields are scanned for named type references inside simple and compound type expressions, including pointers, selectors, slices, maps, and channels; built-in types and unindexed external package types are ignored. In PR graphs, a changed head struct can link through `uses_type` to an unchanged type node from the base graph when that referenced type was not re-written at the PR head SHA.
